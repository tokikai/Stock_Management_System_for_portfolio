...
def fetch_ai_prediction_materials():
    ...
    # 必要なカラムを取得
    cur.execute(
        "SELECT id, created_at, source_url, content_text, meta_json, memo FROM ai予測素材 ORDER BY created_at DESC LIMIT 5;"
    )
    rows = cur.fetchall()
    materials = []
    for row in rows:
...
def get_chatgpt_comment():
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "あなたは別府市のとあるホテルの混雑状況予測アドバイザーです。",
            },
            {
                "role": "user",
                "content": prompt,
                # URLを渡してみる
                # "content": "https://beppu-city.viewer.kintoneapp.com/public/5df5920b6387aa005d2f4014babd20effd4078457c25c71829aaad5aa3c7778d をみて、何月何日に何が開催されるか明らかにして、ホテルの混雑予測レベルを5段階で推論してください。理由も簡潔に述べてください。",
            },
        ],
    )
